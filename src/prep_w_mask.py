...
def main(args):
    with open(os.path.join(args.src_root,"test.json")) as f:#split\\test.json
        pairs = json.load(f)
    with open(os.path.join(args.src_root,"val.json")) as f:
        pairs += json.load(f)
    with open(os.path.join(args.src_root,"train.json")) as f:
        pairs += json.load(f)
    video_ps = []
    for a,b in pairs:
        if args.fake_type == "real":
...
if __name__ == '__main__':
    ...
    arg('--src-root', type=str, default='D:\\code\\CORE-main\\data\\FaceForensics++')
    arg('--dst-root', type=str, default='D:\\code\\CORE-main\\data\\FF_mask')
